...
public class ClientCnxn {
        ...
        public void run() {
           try {
              ...
              while (true) {
                 ...
                 if (wasKilled)
                    synchronized (waitingEvents) {
                        // 只有当waitingEvents为空时，才将运行状态置为false，然后break退出循环
                        // 这里可以看出，即使收到kill事件，也会等到waitingEvents队列为空才会退出，
                        // 否则下一次循环，还会执行processEvent方法来处理事件
    ...
    protected void finishPacket(Packet p) {
        ...
        if (p.cb == null) {
            synchronized (p) {
                p.finished = true;
                // 收到服务端返回的数据，让p.wait()的线程往下执行
                p.notifyAll();
            }
        } else {
            p.finished = true;
            // event事件，交给eventThread处理
    ...
    class SendThread extends ZooKeeperThread {
        ...
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
            ...
            // 代表是服务端发送过来的触发watcher的事件，交给eventThread.queueEvent处理
    ...
    public Packet queuePacket(RequestHeader h, ReplyHeader r, Record request,
        ...
        synchronized (state) {
            if (!state.isAlive() || closing) {
                ...
                // 将请求加入到outgoingQueue队列中，等待发送给客户端
                outgoingQueue.add(packet);
            }
        }
        // 有请求需要发送了，立马唤醒等待的select()，防止等到超时才处理
