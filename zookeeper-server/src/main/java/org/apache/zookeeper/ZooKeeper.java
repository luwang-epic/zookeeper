...
public class ZooKeeper implements AutoCloseable {
    ...
    public ZooKeeper(String connectString, int sessionTimeout, Watcher watcher,
        ...
        // 启动sendThread和eventThread，和服务端进行交互
        cnxn.start();
    }
    ...
    public byte[] getData(final String path, Watcher watcher, Stat stat)
        throws KeeperException, InterruptedException
     {
        ...
        /*
        这里的处理流程是这样的：
            1. 客户端将请求放入outgoingQueue队列中，让sendThread发送给服务器
            2. 发送时，会从outgoingQueue出队，将发送给服务器的请求加入到pendingQueue中，
            3. 客户端等待获取服务器的响应，调用请求对象的wait方法，
            4. 收到响应后，会从pendingQueue中出队，执行请求对象的notifyAll方法，放行
            整个过程中，请求对象都是一样的，对象中包含应答数据，可以判断服务器是否返回等

         对于watcher的处理流程：
            1. watcher会随着请求一起发送到服务器端
            2. 当收到服务器端的数据请求后，会将watcher注册到本地ZKWatchManager中统一管理
            服务器在处理请求时检测到请求体里的watch=true，就在内存数据库里注册一个watcher；
            数据发生变更，就取出该节点上注册的所有watcher，进行触发，触发的动作由服务端传递给客户端；
            客户端也保存了节点和watcher的关系，客户端从内存中取出该节点的所有watcher，
