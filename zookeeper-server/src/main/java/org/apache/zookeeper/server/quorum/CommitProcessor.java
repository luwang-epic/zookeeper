...
public class CommitProcessor extends ZooKeeperCriticalThread implements
        ...
        try {
            while (!stopped) {
                ...
                while (!stopped && !isWaitingForCommit() &&
                    ...
                    // 不是事务请求，直接交给下一个处理器;
                    // 对于leader节点是：Leader.ToBeAppliedRequestProcessor，对于follower节点是FinalRequestProcessor
                    } else {
    ...
    protected void processCommitted() {
        ...
        if (!stopped && !isProcessingRequest() &&
            ...
            /*
              committedRequests是通过follower来接受leader的信息，
              当follower接受到leader的commit信息后，会将提案的信息放入到committedRequests这个队列中
              这里从committedRequests队列中获取已经提交的提案，和需要pending的提案对比，
                    如果一样，将会提交这个提案，即进行事务操作（交给FinalRequestProcessor）
                    如果不一样，说明来自其他机器，直接发送给FinalRequestProcessor操作（注：我没有想到这个会在什么场景下触发）
             */
            if (pending != null &&
    ...
    /**
     * Follower的processPacket方法会调用这个方法，
     * 将leader发来的commit request放到committedRequests队列中
