...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    public synchronized void startup() {
        ...
        // 清理过期的客户端会话
        startSessionTracker();
        // 处理客户端请求，通过RequestProcessor链来处理请求
        // ServerCnxnFactory只是复制对数据的读写和通信以及会话的建立等，不会具体处理请求
        // ServerCnxnFactory WorkService会调用submitRequest方法将客户端请求转发到第一个处理器中
        // 这里处理对zk节点的增删改查等，
    ...
    protected void setupRequestProcessors() {
        ...
        // 这个处理器专门处理log日志的，记录事务日志，保证事务日志持久化到磁盘再调用下一个处理器
        RequestProcessor syncProcessor = new SyncRequestProcessor(this,
                finalProcessor);
        ((SyncRequestProcessor)syncProcessor).start();

        // 这个是第一个处理器，会对请求做一些预处理工作，比如：检查路径，检查权限，开启事务等
        // 调用nextProcessor.processRequest(request);将处理后的请求交给下一个处理器，即：SyncRequestProcessor
