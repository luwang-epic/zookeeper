        ...
        for (FileOutputStream log : streamsToFlush) {
            ...
            if (forceSync) {
                ...
                上面的flush和getChannel().force的差别：
                    一个是应用级一个是磁盘级，当调用flush会将应用数据缓冲区中的全部提交给磁盘驱动去调度，
                    但此时也未必全部同步到磁盘
                    磁盘写一般是异步的，所以后者会保证全部同步到磁盘，类似操作系统的APIfsync
                 */
    ...
    public static class FileTxnIterator implements TxnLog.TxnIterator {
        ...
        void init() throws IOException {
            ...
            for (File f: files) {
                ...
                // 这里将小于该事务id的最大log文件加入到集合中，因为log文件中的操作事务id都比文件名的事务id大
                // 这个逻辑可以看FileTxnLog.append方法
                // 所以比zxid小的最大的一个log中可能包含zxid事务的操作，这些操作也需要回放
         ...
         * @param logFile file input stream associated with the input archive.
         * @throws IOException
         **/
        ...
        public boolean next() throws IOException {
            ...
            try {
                // 当这个文件被读取过时，将会抛出EOFException异常，
                // 该异常会被捕获处理，处理逻辑中将ia指向下一个log文件
