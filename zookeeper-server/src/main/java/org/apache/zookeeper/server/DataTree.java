...
public class DataTree {
    ...
    public byte[] getData(String path, Stat stat, Watcher watcher)
        ...
        synchronized (n) {
            n.copyStat(stat);
            if (watcher != null) {
                // 在服务器端注册watcher，其实watcher就是一个连接对象：ServerCnxn
                // 服务端不关心这个watcher需要怎么处理，这个是客户端需要做的
                // 服务端只需要关注某个路径是否有watcher即可
                // 因此在客户端向服务端发送watcher时，只是发送了这个路径是否有watcher
                // 服务器触发时，也只需要告诉客户端发送了什么事件，怎么处理需要客户端来做
                // 因为客户端保存了watcher触发时，事件处理逻辑
