...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public static class QuorumServer {
        ...
        // 该ip机器上启动的zk状态（PARTICIPANT：参与投票，可成为leader，OBSERVER：不参与投票，不能成为leader）
    ...
    public synchronized void start() {
        ...
        // 开始选主，包括发送选票和接受处理选票
        startLeaderElection();
        // 启动线程，运行run方法，会根据服务器状态选择不同的操作
        super.start();
    }

    private void loadDataBase() {
        try {
            // 从log和snapshot中加载数据到内存
            zkDb.loadDataBase();
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            ...
            // QuorumCnxManager这个类用于管理和其他机器的连接通讯，包括发送和接受选票
            QuorumCnxManager.Listener listener = qcm.listener;
            if(listener != null){
                // 启动监听，等待其他机器连接；
                // zk都是采用sid大的机器向sid小的机器连接，sid小的机器等待比较大的机器连接，然后建立通信
                // 因为在QuorumCnxManager的startConnection中有判断sid的大小
                listener.start();

                // 用于产生选票，并分析处理选票；接受来自QuorumCnxManager中传递过来的选取，进行分析
                // 将产生的选票放到QuorumCnxManager的队列中，交给QuorumCnxManager发送给其他的机器
                FastLeaderElection fle = new FastLeaderElection(this, qcm);
                // 启动WorkerSender发送选票线程和WorkerReceiver接受选票线程
                fle.start();
                le = fle;
            } else {
    ...
    public void run() {
        ...
        try {
            /*
            Java Management Extensions（JMX）技术是 Java SE 平台的标准功能，
            提供了一种简单的、标准的监控和管理资源的方式，对于如何定义一个资源给出了明确的结构和设计模式，
            主要用于监控和管理 Java 应用程序运行状态、设备和资源信息、Java 虚拟机运行情况等信息
             */
            ...
            while (running) {
                ...
                case LOOKING:
                    ...
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        // 在没有确定leader之前，客户端只能读
                        // 只读模式通过设置第一个处理器为ReadOnlyRequestProcessor来实现
                        ...
                        try {
                            ...
                            if (shuttingDownLE) {
                                ...
                                // 如果leader选取已经关闭，开启选主（当节点重启或者异常等状态变为Looking时会调用）
                                startLeaderElection();
                            }

                            // lookForLeader会一直运行，直到leader确定，将当前投票设置最后的投票，包括leader等信息
                ...
                case OBSERVING:
                    try {
                        LOG.info("OBSERVING");
                        // 确定该节点zk状态为observer，启动相关服务
                        // 和follower相比，不需要设置投票相关的信息
